...
def dice_coef_edema(y_pred,y_true,epsilon=1e-6):
    y_pred_f = torch.flatten(y_pred[:,2,:,:])
    y_true_f = torch.flatten(y_true[:,2,:,:])
    intersection = torch.sum(y_pred_f*y_true_f)+epsilon
    union = torch.sum(y_pred_f)+torch.sum(y_true_f)+epsilon
    loss = 2*intersection/union
    total_loss += loss  
...
def dice_coef_enchancing(y_pred,y_true,epsilon=1e-6):
    y_pred_f = torch.flatten(y_pred[:,3,:,:])
    y_true_f = torch.flatten(y_true[:,3,:,:])
    intersection = torch.sum(y_pred_f*y_true_f)+epsilon
    union = torch.sum(y_pred_f)+torch.sum(y_true_f)+epsilon
    loss = 2*intersection/union
    total_loss += loss
...
class Mydataset(Dataset):
    def __init__(self,volume_savingpath,volume_namelist,slice_start=22,slice_num = 100,transforms=None):
    ...
    def __getitem__(self,index):
        ...
        for j in range(self.slice_num): # slice num=100
            t1ce = torch.from_numpy(cv2.resize(t1ce_array[:,:,j+self.slice_start],(160,160)))
            flair = torch.from_numpy(cv2.resize(flair_array[:,:,j+self.slice_start],(160,160)))
...
class Unet(nn.Module):
    ...
    def forward(self,x):
        ...
        skip5 = nn.Dropout2d(p=0.1)(skip5)
...
class Loss(nn.Module):
    def __init__(self,mode,smooth=1e-7,dice_weight=0.5):
        super().__init__()
        self.mode = mode
        self.smooth = smooth
    ...
    def forward(self,y_pred,gt):
        ...
        y_true_onehot = F.one_hot(gt,num_classes = class_num).permute(0,3,1,2) #NCHW
        intersection = torch.sum(y_prob*y_true_onehot,dim=(2,3))
        union = torch.sum(y_prob,dim=(2,3))+torch.sum(y_true_onehot,dim=(2,3))+self.smooth
        dice = (2*intersection+self.smooth)/union
        dice_loss = 1-dice.mean()

        y_prob_cce = F.softmax(y_pred,dim=1).permute(0,2,3,1)  #NHWC
        y_prob_cce = torch.clamp(y_prob_cce,min=1e-7,max=1)
        y_true_onehot_cce = F.one_hot(gt,class_num) #NHWC
        CCE_loss = (-1*torch.sum(torch.log(y_prob_cce)*y_true_onehot_cce,dim=-1)).mean()
        ...
        if self.mode =='dice':
            ...
            raise ValueError(f"Invalid mode {self.mode}, choose from ['dice','combine','cce']")
...
ds_train = Mydataset(volume_saving_path,train_names,transforms=t_transform)
...
loss_fn = Loss(mode='combine',dice_weight=0.7)
epoch = 70
lr_scheduler=optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='max',factor=0.5,patience=5,min_lr=0.000001)
...
# checkpoint = torch.load('segmentation_epoch22_lr0.001_per_patient_batch.pth',weights_only=True)
# model = Unet(encoder1,encoder2,encoder3,encoder4,encoder5,decoder1,decoder2,decoder3,decoder4)
# model.load_state_dict(checkpoint['model'])

# t1ce,flair,mask,preds=model_test(test_names,model,9,50,t_transform)
